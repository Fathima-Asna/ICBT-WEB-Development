...
<%
    // ── Session Guard: already logged in? Send to their dashboard ────────────
    HttpSession existingSession = request.getSession(false);
    if (existingSession != null && existingSession.getAttribute("userRole") != null) {
        String role = (String) existingSession.getAttribute("userRole");
        if ("Admin".equals(role))    { response.sendRedirect(request.getContextPath() + "/admin-dashboard.jsp"); return; }
        if ("Staff".equals(role))    { response.sendRedirect(request.getContextPath() + "/staff/action"); return; }
        if ("Customer".equals(role)) { response.sendRedirect(request.getContextPath() + "/customer-dashboard.jsp"); return; }
    }

    // ── Retrieve server-side error / repopulation data ─────────────────────
    ...
    // Helper to safely render repopulated values
...
<head>
    ...
    <meta name="description" content="Register an account with GlobeTrek Adventures. Start designing bespoke handcrafted travel itineraries and luxury stays.">
    <title>Join GlobeTrek Adventures | Create Account</title>
...
<body>
    ...
    <header>
        <div class="container header-container">
            ...
            <ul class="nav-menu" id="navigation-links">
                <li><a href="<%= request.getContextPath() %>/index.jsp" class="nav-link" id="nav-home">Home</a></li>
            ...
            <div class="nav-cta" id="header-cta-buttons">
                ...
                <a href="<%= request.getContextPath() %>/signup.jsp" class="btn btn-primary" style="padding: 10px 22px; font-size: 0.85rem; border-color: var(--secondary);" id="cta-signup">Sign Up</a>
            </div>
        </div>
    </header>
    ...
    <main>
        <section class="split-layout" id="signup-section">
            
            <!-- Left Side: Registration Form -->
            <div class="form-side" id="signup-form-container">
                <div class="form-header">
                    <h1>Create Your Adventure Account</h1>
                    <p>Already a member? <a href="<%= request.getContextPath() %>/login.jsp" id="link-to-login">Log in here</a> to view your booked expeditions.</p>
                </div>

                <%-- ── Error banner (server-side validation failure) ───────────── --%>
                <% if (errorMsg != null && !errorMsg.isEmpty()) { %>
                <div class="auth-banner auth-banner--error" id="signup-error-banner" role="alert">
                    <svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>
                    <%= errorMsg %>
                </div>
                <% } %>

                <!-- Structured Standard Form connected to AuthServlet -->
                <form action="<%= request.getContextPath() %>/auth" method="POST" autocomplete="on" id="signup-form">
                    <!-- Hidden action discriminator -->
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-wrapper">
                                <input type="text" id="first-name" name="first_name" class="form-input"
                                       placeholder="e.g. Emily" required
                                       value="<%= firstName %>">
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="input-wrapper">
                                <input type="text" id="last-name" name="last_name" class="form-input"
                                       placeholder="e.g. Watson" required
                                       value="<%= lastName %>">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            <input type="email" id="email" name="email" class="form-input"
                                   placeholder="e.g. emily.watson@globe.com" required
                                   value="<%= email %>">
                            <span class="form-helper">Please enter a valid email address.</span>
                        </div>
                    </div>
                    ...
                    <div class="form-group-row">
                        <div class="form-group">
                            ...
                            <div class="input-wrapper">
                                ...
                                <span class="form-helper">Password must be at least 8 characters.</span>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="input-wrapper">
                                <input type="password" id="confirm-password" name="confirm_password"
                                       class="form-input" placeholder="Retype password" minlength="8" required>
                            </div>
                        </div>
                    </div>

                    <!-- Dropdown Selection (Travel Preferences) -->
                    <div class="form-group">
                        <label class="form-label" for="travel-style">Preferred Travel Style</label>
                        <div class="input-wrapper">
                            <select id="travel-style" name="travel_style" class="form-input form-select" required>
                                <option value="" disabled <%= travelStyle.isEmpty() ? "selected" : "" %>>Select what inspires you...</option>
                                <option value="alpine"   <%= "alpine".equals(travelStyle)   ? "selected" : "" %>>Alpine Trekking &amp; Peaks</option>
                                <option value="tropical" <%= "tropical".equals(travelStyle) ? "selected" : "" %>>Tropical Sand &amp; Coral Reefs</option>
                                <option value="cultural" <%= "cultural".equals(travelStyle) ? "selected" : "" %>>Heritage &amp; Cultural Ancient Trails</option>
                                <option value="eco"      <%= "eco".equals(travelStyle)      ? "selected" : "" %>>Eco Lodging &amp; Wildlife Safaris</option>
                                <option value="luxury"   <%= "luxury".equals(travelStyle)   ? "selected" : "" %>>Ultra-Luxury Boutique Escapes</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-options">
                        <label class="checkbox-container" id="terms-checkbox-label">
                    ...
                    <!-- Submit Button -->
                    <button type="submit" class="btn btn-primary submit-btn" id="btn-create-account">Create Free Account</button>
                </form>
            </div>

            <!-- Right Side: Immersive Travel Image Vibe -->
            <div class="image-side" style="background-image: url('https://images.unsplash.com/photo-1507525428034-b723cf961d3e?q=80&w=1000&auto=format&fit=crop');" id="signup-image-side">
                <div class="image-side-content">
                    <h2>Leave Footprints.</h2>
                    <p>Join over 15,000 global citizens who trust GlobeTrek Adventures to map their life-defining moments.</p>
                </div>
            </div>
        ...
        <div class="container">
            ...
            <div class="footer-bottom">
                <div class="footer-copyright">
                    &copy; 2026 GlobeTrek Adventures Ltd. All rights reserved.
                </div>
                <ul class="footer-policies">
                    <li><a href="<%= request.getContextPath() %>/index.jsp">Privacy Policy</a></li>
                    <li><a href="<%= request.getContextPath() %>/index.jsp">Terms of Service</a></li>
                    <li><a href="<%= request.getContextPath() %>/index.jsp">Sitemap</a></li>
                </ul>
            </div>
        </div>
    </footer>
