...
<%
    ...
    // ── Retrieve server-side error or status messages ─────────────────────────
    String errorMsg    = (String) request.getAttribute("errorMsg");
    String logoutParam = request.getParameter("logout");
    String errorParam  = request.getParameter("error");

    // Map error query params to human-readable messages
    if (errorMsg == null && errorParam != null) {
        switch (errorParam) {
            case "unauthorized":    errorMsg = "Please log in to access that page."; break;
            case "forbidden":       errorMsg = "You do not have permission to view that page."; break;
            case "session_expired": errorMsg = "Your session has expired. Please sign in again."; break;
        }
    }
%>
<!DOCTYPE html>
...
<head>
    ...
    <meta name="description" content="Log in to GlobeTrek Adventures to access your custom itineraries, premium stays, and personal concierge.">
    <title>Welcome Back | GlobeTrek Adventures Login</title>
...
<body>
    ...
    <header>
        <div class="container header-container">
            ...
            <div class="nav-cta" id="header-cta-buttons">
                <a href="<%= request.getContextPath() %>/login.jsp" class="btn btn-outline" style="padding: 10px 22px; font-size: 0.85rem; border-color: var(--secondary); color: var(--secondary);" id="cta-login">Login</a>
    ...
    <main>
        <section class="split-layout" id="login-section">
            
            <!-- Left Side: Login Form -->
            <div class="form-side" id="login-form-container">
                <div class="form-header">
                    <h1>Welcome Back, Traveler</h1>
                    <p>New to GlobeTrek? <a href="<%= request.getContextPath() %>/signup.jsp" id="link-to-signup">Create an account</a> to join our wanderlust circle.</p>
                </div>

                <%-- ── Logout success banner ──────────────────────────────────── --%>
                <% if ("true".equals(logoutParam)) { %>
                <div class="auth-banner auth-banner--success" id="logout-success-banner" role="alert">
                    <svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
                    You have been successfully signed out. See you next adventure!
                </div>
                <% } %>

                <%-- ── Error banner ────────────────────────────────────────────── --%>
                <% if (errorMsg != null && !errorMsg.isEmpty()) { %>
                <div class="auth-banner auth-banner--error" id="login-error-banner" role="alert">
                ...
                <form action="<%= request.getContextPath() %>/auth" method="POST" autocomplete="on" id="login-form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            <input type="email" id="email" name="email" class="form-input" placeholder="e.g. emily@watson.com" required>
                    ...
                    <!-- Extra details (Remember me & Forgot Password) -->
                    <div class="form-options">
                        <label class="checkbox-container" id="remember-checkbox-label">
                            Remember my session
                            <input type="checkbox" name="remember_me">
                            <span class="checkmark"></span>
                        </label>
                        <a href="<%= request.getContextPath() %>/contact.html" class="forgot-link" id="link-forgot-password">Forgot Password?</a>
                    </div>

                    <!-- Submit Button -->
                    <button type="submit" class="btn btn-primary submit-btn" id="btn-login-submit">Sign In Account</button>
                </form>
            </div>

            <!-- Right Side: Immersive Mountain Travel Image -->
            <div class="image-side" style="background-image: url('https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?q=80&w=1000&auto=format&fit=crop');" id="login-image-side">
                <div class="image-side-content">
                    <h2>Track Your Journeys.</h2>
                    <p>Access custom itinerary maps, exclusive hotel upgrades, and flight concierge updates instantly.</p>
                </div>
            </div>
